      ...
      if (firstname) {
        if (filter && !filter == "") {
          if (filter === "From to End") {
            const leaveFound = await UserLeave.find({
              name,
              $and: [
            ...
            return res.json({
              data: newleaves,
              allLeaves: leaveFound,
              length: leaveFound.length,
            });
          } else {
            const leaveFound = await UserLeave.find({
              name,
...
adminRouter.get("/allData",async(req,res)=>{
  ...
  try{
    ...
    const superManager = await Manager.find({superManagerId:""});

    const employees = await User.find({});

    const manEmployees = await Manager.find({superManagerId:{$ne:""}});

    const allEmployees  = manEmployees.concat(employees);

    return res.json({allEmployeesData:allEmployees,supermanagerData:superManager});
